...
- **VLA-adjacent 基础模型**: 视觉 token backbone（Being-VL-0/0.5）和人体 motion foundation model（Being-M0/M0.5）不直接输出机器人 action，但为 VLA 的感知/grounding 和 humanoid motion prior 提供上游能力
- **层次化 VLA (Hierarchical VLA)**: 统一模型同时做高层语义推理+低层动作生成（π₀.5, Hi Robot, **MEM**）
- **VLM-as-Controller 系统框架 (Agentic Robotics)**: 现成VLM做高层元控制器，调度低层VLA/技能策略；分离式层次化编排（SayCan, Inner Monologue, **Being-0**, **RoboClaw**）
- **世界模型 + VLA**: 预测未来状态再规划动作，四类流派：隐式/latent query（FLARE, **Being-H0.7**）、联合去噪(**Cosmos Policy**, **DreamZero**, UWM)、视频→IDM(UniPi, Seer)、**World Model as Data Engine**（EmbodieDreamer/MimicDreamer/EgoDemoGen/GigaWorld-0/GigaBrain-0），以及 world-model RL self-improvement（**RISE**）
...
- **VLA Reasoning / RLVR** 🆕: 用可验证 reward 强化 affordance、几何关系和轨迹推理（**VLA-R1**），连接 chain-of-thought 与机器人动作对齐
- **奖励/价值学习**: 从人类反馈或视频中学习奖励函数；**ViVa** 将视频生成模型改造成 value model 支持长时序机器人 RL
...
- **人形机器人 Loco-Manipulation** 🆕: 全身移动、双臂操作和底层 whole-body control 耦合（EgoHumanoid, Figure Helix 02, Being-0, Being-H0.7）
- **Egocentric Hand Motion / Perception** 🆕: 手部运动重建、预测、生成和第一人称视频理解，为 human-data VLA 提供中间监督（HaWoR, Uni-Hand, MEgoHand, OpenMMEgo）

→ 详见 [tasks/index.md](tasks/index.md)

## 技术组件
- **动作表示**: 离散 token / 连续向量 / action chunk / FAST tokenizer / 混合方案 / **task-centric latent action**（UniVLA：action-free video→潜动作token）/ 人手 MANO motion token（Being-H0）/ 混合连续flow+离散masked prediction（Being-H0.5）/ **latent frame injection**（Cosmos Policy：动作编码为 latent frame 插入视频 diffusion 序列）/ 人体 motion token（Being-M0/M0.5，VLA-adjacent）
...
- **视觉编码器 / 视觉 tokenization**: SigLIP, DINOv2, ViT, ViT-22B (PaLI-X), ViT-4B (PaLM-E), 浅层CNN patch encoder; SigLIP+DINOv2 融合（OpenVLA/Prismatic 验证有效）; **视觉 BPE token**（Being-VL-0/0.5：VQ token + BPE/priority-guided BPE）; **Wan2.1 Spatiotemporal VAE**（DreamZero + Cosmos Policy）; **视频编码器**（MEM：空间-时间分离注意力 ViT，16帧<300ms，不引入新参数）; **4D geometry teacher**（SwiftVLA：训练期蒸馏、推理期移除）; **egocentric LMM perception**（OpenMMEgo）🆕
...
- **架构创新**: Mixture-of-Flow（MoF，Being-H0.5：共享基础层+路由特化专家）、流形保持门控（MPG）、通用异步分块（UAC）、**Latent WAM queries + prior/posterior alignment**（Being-H0.7：未来观测监督压入部署期 latent）、**Generalist + Diffusion Specialist 双系统**（RoboDual：OpenVLA 语义泛化 + 小 DiT 高频控制）、**Latent Frame Injection + Conditioning Mask**（Cosmos Policy：无架构修改的多模态适配）、**闭环 KV Cache 替换**（DreamZero：用真实观测替换预测帧消除累积误差，WAM 独有优势）、**System 1/2/0 humanoid hierarchy**（Figure Helix：官方技术页）、**action-centered WAM**（GigaWorld-Policy）🆕
- **数据策略**: cross-embodiment, co-training/co-fine-tuning (RT-2首创), verbal instruction, augmentation, **人类中心学习**（Being-H0/H0.5/H0.7: 人手/ego 视频作为物理母语与未来动态先验）, **无标注真实视频 V-L-A 化**（VITRA：3D手/相机重建+速度极小值切分+trajectory-overlay captioning→1M episodes）, **机器人数据多样性 scaling**（GO-1-Pro：task diversity 最关键，expert diversity 需 debias）, **触觉/硬件数据闭环**（MM-Hand/TAMEn/FreeTacMan）, **Ego human data + anchor data**（EgoVerse/EgoScale/EMMA/EgoHumanoid：大规模多样ego数据需少量对齐锚点）, **World Model as Data Engine**（EmbodieDreamer/MimicDreamer/EgoDemoGen/GigaWorld-0/GigaBrain-0：合成 paired video-action 数据）, **EAP自重置数据收集**（RoboClaw: 正-逆动作对实现自主数据循环采集）, **Rollout Experience Learning**（Cosmos Policy/RISE：从 rollout 或 imagined rollout 微调世界模型/价值函数/策略）, **多样性优先非重复数据 + 任务淘汰机制**（DreamZero：颠覆传统重复示教范式）, **人类干预修正数据**（MEM：失败→修正数据训练上下文适应）🆕, **倒带精修（Rewind-and-Refine）+ Task Sentinel 自动监控**（GCENT：关节状态缓冲倒带失效点 + 步骤成功检测，1:N 可扩展，数据效率提升 2.25x）🆕
...
- **价值/推理后训练**: **RAMP**（GigaBrain-0.5M*：future state/value 条件策略）, **video-generative value model**（ViVa）, **step-wise contrastive ranking**（π-StepNFT）, **RLVR/GRPO reasoning**（VLA-R1）
  ...
  → Octo (2024.01, 开源GRP奠基, 将diffusion head扩展到跨具身预训练)
    ...
    → OpenVLA (2024.06, 开源VLA奠基, VLM+Action Token, 7B超越RT-2-X 55B)
      → π₀ (2024.10, VLM+Flow Head, flow matching替代DDPM, 解决OpenVLA的离散化/action chunking局限) 
        ...
        └── GigaBrain-0/0.5M* (2025-2026, world model data engine + RAMP RL 后训练)
...
Human/Ego Video → Robot Policy (人类数据预训练路线)
  EgoMimic (2024, AR眼镜ego数据co-training奠基)
    ...
    → UniDex/EgoVerse (2026, 灵巧手FAAS统一动作空间 / 全球ego数据集+anchor data结论)
...
Video Model → Policy (视频预训练→机器人策略, 世界模型路线)
  ...
  GigaWorld 系列 (GigaAI, world model as data engine)
    → EmbodieDreamer / GigaWorld-0 (2025, Real2Sim2Real + synthetic paired video-action data)
    → GigaWorld-Policy (2026, action-centered WAM, 推理时action-only低延迟控制)
...
SERL (2024, Q-function RL for robot)
  ...
  → π-StepNFT (2026, critic-free/likelihood-free flow RL, step-wise contrastive ranking)
  → GigaBrain-0.5M* / ViVa (2026, world model future state/value conditioning / video-generative value model)
...
TinyVLA (2024, 子10亿参数, 仍需机器人预训练) → SmolVLA (2025, 2.25B, Meta-World 68.2%)
  ...
  → SwiftVLA (2025, 0.45B, 训练期4D时空几何蒸馏, 推理期轻量部署)
