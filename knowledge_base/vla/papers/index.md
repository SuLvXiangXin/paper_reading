...
| **Being-H0** | **2025** | **Human Data Pretraining + Motion-token VLA** | **把 UniHand 人类手部视频转成 MANO motion token 指令数据，通过 physical instruction tuning + part-level GRQ tokenizer 学习人手操作先验，再用少量 Franka+Inspire 后训练迁移到灵巧手任务，是 Being-H 人类中心路线的起点** | **[being_h0_2025.md](being_h0_2025.md)** |
...
| **Being-H0.7** | **2026** | **Latent World-Action Model + Ego Video Scaling** | **用 200K 小时 ego 视频 + 15K 小时机器人数据训练 latent query world-action workspace，通过 prior/posterior future alignment 把未来观测信息压入可部署 latent，推理时无需像素 rollout，在动态、物理和长时序真机 suite 上领先 H0.5/π0.5/Fast-WAM** | **[being_h07_2026.md](being_h07_2026.md)** |
...
| Being-VL-0 | 2024 | MLLM Backbone / Visual BPE Tokenization | 用 VQ-GAN + 2D BPE 把图像离散成可与语言同序列建模的视觉 token，证明视觉 tokenizer 可作为 VLM/VLA backbone 的上游组件，但不输出机器人动作 | [being_vl0_2024.md](being_vl0_2024.md) |
| Being-VL-0.5 | 2025 | MLLM Backbone / Priority-guided Visual BPE | 在 Being-VL-0 上加入 spatial consistency、curriculum data composition 和 progressive unfreezing，把离散视觉 token MLLM 推进到更强 multimodal understanding，可作为 VLA 感知/grounding backbone 候选 | [being_vl05_2025.md](being_vl05_2025.md) |
| Being-M0 | 2024 | Human Motion Foundation Model | 用百万级 MotionLib 和 MotionBook/2D-LFQ 训练 LLM 式 motion token 生成器，提供 humanoid/whole-body policy 可借用的人体运动先验，但不是闭环 robot policy | [being_m0_2024.md](being_m0_2024.md) |
| Being-M0.5 | 2025 | Vision-Language-Motion Model / Motion Prior | 基于 HuMo100M、PRQ 部位级 tokenizer 和 7B VLMM 实现实时可控人体运动生成，可作为 humanoid VLA 的高层 motion prior、teacher 或数据增强来源 | [being_m05_2025.md](being_m05_2025.md) |
| Being-0 | 2025 | VLM-as-Controller / Humanoid Skill Agent | 云端 FM 做低频任务分解，本地 VLM Connector 做目标 grounding 和技能选择，RL/ACT modular skills 做高频控制，是 humanoid 长时序任务的分离式 agentic 系统路线 | [being_0_2025.md](being_0_2025.md) |
