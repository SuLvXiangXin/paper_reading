...
- **VITRA** (2025, MSRA/Tsinghua) — 无标注真实人类活动视频 → 自动 V-L-A episode 构造，1M atomic hand-action episodes
- **Being-H0/H0.5/H0.7** (2025-2026, BeingBeyond) — 从 UniHand motion-token VLA、35K+ 小时跨具身 flow VLA，扩展到 200K 小时 ego video latent WAM
...
5. **无标注视频 V-L-A 化**: VITRA 用 3D hand/camera reconstruction、world-space wrist speed minima 切分、trajectory-overlay GPT caption，把无脚本 real-life videos 转成 atomic V-L-A episode
6. **视角与具身锚点**: EgoVerse/EgoScale/EMMA/VITRA 显示，大规模多样 ego 数据通常还需要少量 domain-aligned human-robot anchor data 或目标机器人数据，把人类视角、尺度和动作接口锚定到 robot policy
7. **功能-执行器动作对齐**: UniDex 的 FAAS 将不同灵巧手按功能部位和 actuator 对齐，是 Being-H0.5 全具身统一动作空间之外的灵巧手专用统一表示
...
EgoBridge/EgoVLA/EgoZero (2025): 扩展到 domain adaptation / VLA 预训练 / smart glasses
  ↓
VITRA (2025): 无标注真实人类活动视频 → 自动切分、caption、3D动作标注 → 1M V-L-A episodes
  ↓
...
Being-H0.5 / EgoScale / UniDex / EgoVerse (2026): 万小时级 scaling、统一动作空间、全球 ego 数据集，验证 transfer 是 scaling + alignment phenomenon
  ↓
Being-H0.7 (2026): 200K 小时 ego video + 15K 小时 robot demos，将 human-data scaling 从直接 VLA 推进到 future-aware latent WAM
```
